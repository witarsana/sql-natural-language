...
import { queryRequestSchema, QueryRequest, QueryResponse } from '../models/query.model';
...
export default async function queryRoutes(fastify: FastifyInstance) {
  ...
  fastify.post<{ Body: QueryRequest }>(
      ...
      schema: {
        body: {
          ...
          properties: {
            ...
            role: { type: 'string', enum: ['Admin', 'Sales', 'Operations', 'Management'] },
    ...
    async (request: FastifyRequest<{ Body: QueryRequest }>, reply: FastifyReply) => {
      ...
      try {
        ...
        // Step 1: Build and validate SQL query using AI
        const aiResponse = await queryBuilderService.buildQuery(
          validatedData.question,
          validatedData.role
        );

        // Step 2: Additional sanitization
  ...
  fastify.get('/examples', async (request: FastifyRequest, reply: FastifyReply) => {
    const examples = [
      {
      ...
      tables: [
        {
          ...
          commonFields: ['plot_id', 'plot_number', 'plot_section_id', 'plot_status_id', 'plot_area'],
        },
        {
          name: 'graves',
          description: 'Burial records',
          commonFields: ['grave_id', 'grave_plot_id', 'grave_person_id', 'grave_burial_date'],
        },
        {
          name: 'persons',
          description: 'Deceased and contact persons',
          commonFields: ['person_id', 'person_first_name', 'person_last_name', 'person_date_of_death'],
        },
        {
          name: 'sections',
